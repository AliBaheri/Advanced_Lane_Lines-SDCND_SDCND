...
I then used the output of `objpoints` and `imgpoints` to compute the camera calibration and distortion coefficients using the `cv2.calibrateCamera()` function.  I applied this distortion correction to the test image using the `cv2.undistort()` function and obtained this result: 
...
I followed the simple role from math (and also nited in lecture) to measure the radius of curvature (please see the notebook).
